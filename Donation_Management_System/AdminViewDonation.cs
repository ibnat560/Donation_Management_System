...
{
    public partial class AdminViewDonation : Form
    {
        int userId;
        SqlConnection con;
        public void sqlcon()
        ...
        public AdminViewDonation(int userId)
        {
            InitializeComponent();
            this.userId = userId;
        }
        ...
        private void AdminViewDonation_Load(object sender, EventArgs e)
        {
            sqlcon();
            SqlCommand sq1 = new SqlCommand("SELECT * FROM DONATIONS ORDER BY SectorId", con);
            DataTable dt = new DataTable();
            SqlDataAdapter sd = new SqlDataAdapter(sq1);
            sd.Fill(dt);
            dataGridView1.DataSource = dt;
            label2.Text = "Number Of Donations: " + dt.Rows.Count.ToString();
            con.Close();
